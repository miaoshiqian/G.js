...
SWFUpload.prototype.initSettings = function (userSettings) {
    ...
    this.ensureDefault("upload_resize_complete_handler", null);
    this.ensureDefault("resize_progress_handler", null);
...
SWFUpload.prototype.loadFlash = function () {
    ...
    targetElement = this.settings.button_placeholder;
    ...
    if (targetElement.currentStyle) {
        wrapperType = targetElement.currentStyle["display"] === "block" ? "div" : "span";
    } else if (window.getComputedStyle) {
        if (document.defaultView.getComputedStyle(targetElement, null)) {
            wrapperType = document.defaultView.getComputedStyle(targetElement, null).getPropertyValue("display") === "block" ? "div" : "span";
        } else {
            wrapperType = "div";
        }
    } else {
    ...
    var flashHTML = this.getFlashHTML();
    targetElement.appendChild(tempParent);
    try {
...
SWFUpload.prototype.getFlashVars = function () {
    ...
    return ["movieName=", encodeURIComponent(this.movieName),
            ...
            "&amp;debugEnabled=", encodeURIComponent(true),
...
SWFUpload.prototype.displayDebugInfo = function () {
    this.debug(
        [
            ...
            "\t", "upload_resize_complete_handler assigned:      ", (typeof this.settings.upload_resize_complete_handler === "function").toString(), "\n",
...
SWFUpload.prototype.stopUpload = function (fileID) {
    this.callFlash("StopUpload", [fileID]);
};
...
SWFUpload.prototype.setSuccessfulUploads = function (successful_uploads) {
    this.settings.successful_uploads = successful_uploads;
    this.callFlash("SetSuccessfulUploads", [successful_uploads]);
};
...
SWFUpload.prototype.flashReady = function (message) {
    try {
        var movieElement = this.cleanUp();
    } catch(ex) {
        this.debug('error in cleanUp')
    }
...
SWFUpload.prototype.uploadResizeComplete = function (file) {
    file = this.unescapeFilePostParams(file);
    this.queueEvent("upload_resize_complete_handler", [file]);
};
...
SWFUpload.prototype.returnUploadStart = function (file) {
    ...
    this.callFlash("ReturnUploadStart", [returnValue, file.id]);
};
...
SWFUpload.prototype.resizeProgress = function (file, bytesComplete, bytesTotal) {
    file = this.unescapeFilePostParams(file);
    this.queueEvent("resize_progress_handler", [file, bytesComplete, bytesTotal]);
};
